...
void method_E1(void) {
  ...
  raio = sqrt((area * 1 / 3) / M_PI);

  // mostrar valores
  printf("O raio um terço dessa área e: %.2lf", raio);
...
void method_E2(void) {
  ...
  // calcular e mostrar o raio de três quartos do volume da esfera e a área de
  // sua superfície
  raio = pow((3.0 / (4.0 * M_PI) * vol * 3.0 / 4.0), 1.0 / 3.0);
  area = M_PI * pow(raio, 2.0);

  // mostrar valores
  printf("O raio de tres quartos do volume e: %.2lf\n", raio);
  printf("A area de sua superficie e: %.2lf\n", area);
