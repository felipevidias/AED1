...
#define ref                                                                    \
  * // para passar parametro por referencia
    // (na assinatura)
#define var                                                                    \
  & // para passar parametro por referencia
    // (na chamada)
...
void IO_debugOFF() { IO_trace = false; } // end IO_debug_OFF ( )

void IO_debugON() { IO_trace = true; } // end IO_debug_ON  ( )
...
void IO_id(const char *const text) {
  IO_printf("%s\n", text);
  IO_printf("%s\n", "Autor: 817294_Felipe_Vilhena_Dias");
  IO_printf("\n"); // mudar de linha
} // fim IO_id ( )
 ...
 */
chars IO_concat(const char *const text1,
                const char *const text2) { // reservar area
 ...
 */
chars IO_toString_b(bool x) { // reservar area
...
/**
    ...
    @param x     - valor real
 */
chars IO_toString_f(double x) { // variante do printf( )
  chars buffer = IO_new_chars(STR_SIZE + 1);
  sprintf(buffer, "%lf", x);
  return (buffer);
} // fim IO_toString_f ( )
